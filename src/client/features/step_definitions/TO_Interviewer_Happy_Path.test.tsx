...
defineFeature(feature, (test) => {
  ...
  test("View live survey list in TOBI", ({ given, when, then }) => {
    ...
    then(
      "I will be able to view all live surveys with questionnaires loaded in Blaise, identified by their three letter acronym TLA, i.e. OPN, LMS",
      async () => {
        await waitFor(() => {
  ...
  test("Select survey", ({ given, when, then, and }) => {
    given("I can view a list of surveys on Blaise within TOBI", async () => {
      mock_server_request(200, survey_list_with_OPN_with_three_active_instruments);
      render(
        <MemoryRouter initialEntries={["/"]}>
          <App />
        </MemoryRouter>,
    ...
    then(
      "I am presented with a list of active questionnaires to be worked on that day for that survey, i.e. within the the survey period start and end dates",
      async () => {
        await waitFor(() => {
  ...
  test("Return to select survey", ({ given, when, then }) => {
    given("I have selected a survey", async () => {
      mock_server_request(200, survey_list_with_OPN_with_two_active_instruments);
      render(
        <MemoryRouter initialEntries={["/"]}>
          <App />
        </MemoryRouter>,
