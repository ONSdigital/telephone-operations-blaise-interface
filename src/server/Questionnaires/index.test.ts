...
import { type BlaiseApiClient, type Questionnaire } from "blaise-api-node-client";
...
describe("QuestionnaireRouter", () => {
  ...
  app.use(QuestionnaireRouter(environmentVariables, blaiseApiMock.object));
  ...
  it("sends the auth header to BIMS", async () => {
    ...
    (getGoogleAuthToken as vi.Mock).mockImplementation(() => Promise.resolve("example-token"));
    blaiseApiMock
      .setup((api) => api.getAllQuestionnairesWithCatiData())
      .returns(async () => [questionnaire]);
    mockHttp.onGet(`http://bims.com/tostartdate/${questionnaire.name}`).reply(({ headers }) => {
      sentHeaders = headers;
  ...
  it("returns the instrument when tostartdate errors and questionnaire is active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      `the instrument ${questionnaire.name} is live for TO (TO start date = Not set) (Active today = true)`,
    );
    expect(log.info).toHaveBeenCalledWith("Retrieved active instruments, 1 item/s");
    expect(log.warn).not.toHaveBeenCalled();
    expect(log.error).toHaveBeenCalledWith(
      "The BIMS request responded with a status of 500 and a body of undefined",
    );
  });
  ...
  it("does not return the instrument when tostartdate errors questionnaire is not active today", async () => {
    ...
    expect(response.body).toEqual([]);
    expect(log.debug).toHaveBeenCalledWith(
      `the instrument ${questionnaire.name} is live for TO (TO start date = Not set) (Active today = false)`,
    );
  ...
  it("returns the instrument when tostartdate response type is not json and questionnaire is active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      "The BIMS request responded with a status of 200 and a body of [object Object]",
    );
    expect(log.debug).toHaveBeenCalledWith(
      `the instrument ${questionnaire.name} is live for TO (TO start date = Not set) (Active today = true)`,
    );
  ...
  it("returns the instrument when tostartdate is today and active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /the instrument OPN2211A is live for TO \(TO start date = .*\) \(Active today = true\)/,
      ),
    );
    expect(log.info).toHaveBeenCalledWith("Retrieved active instruments, 1 item/s");
    expect(log.warn).not.toHaveBeenCalled();
  ...
  it("does not return the instrument when tostartdate is today and not active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /the instrument OPN2211A is live for TO \(TO start date = .*\) \(Active today = false\)/,
      ),
    );
    expect(log.info).toHaveBeenCalledWith("Retrieved active instruments, 0 item/s");
    expect(log.warn).not.toHaveBeenCalled();
  ...
  it("does not return the instrument when tostartdate is tomorrow and active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /the instrument OPN2211A is not currently live for TO \(TO start date = .*\) \(Active today = true\)/,
      ),
    );
    expect(log.info).toHaveBeenCalledWith("Retrieved active instruments, 0 item/s");
    expect(log.warn).not.toHaveBeenCalled();
  ...
  it("does not return the instrument when tostartdate is tomorrow and not active today", async () => {
    ...
    expect(log.debug).toHaveBeenCalledWith(
      expect.stringMatching(
        /the instrument OPN2211A is not currently live for TO \(TO start date = .*\) \(Active today = false\)/,
      ),
    );
    expect(log.info).toHaveBeenCalledWith("Retrieved active instruments, 0 item/s");
    expect(log.warn).not.toHaveBeenCalled();
