...
import { type BlaiseApiClient, type Survey } from "blaise-api-node-client";
import { defineFeature, type DefineStepFunction, loadFeature } from "jest-cucumber";
...
describe("Given the API returns 2 instruments with only one that is active", () => {
  beforeAll(() => {
    blaiseApiMock
      .setup((api) => api.getAllQuestionnairesWithCatiData())
      .returns(async () => apiQuestionnaireList);
    const liveDateUrl = new RegExp(`${environmentVariables.BIMS_API_URL}/tostartdate/.*`);

    mock
      .onGet(liveDateUrl)
      .reply(200, { tostartdate: null }, { "content-type": "application/json" });
  });
  ...
  const questionnairesReturned = [
    {
      survey: "OPN",
      questionnaires: [
        QuestionnaireHelper.Questionnaire(
          "OPN2007T",
          true,
          "July 2020",
          "OPN",
          "https://external-web-url/OPN2007T?LayoutSet=CATI-Interviewer_Large",
        ),
      ],
    },
  ];
...
describe("Given the API returns 2 active instruments for the survey OPN", () => {
  ...
  const questionnairesReturned = [
    {
      ...
      questionnaires: [
        ...
        QuestionnaireHelper.Questionnaire(
          ...
          "April 2020",
          "OPN",
          "https://external-web-url/OPN2004A?LayoutSet=CATI-Interviewer_Large",
        ),
      ],
    },
  ];
  ...
  it("should return a list with one survey with 2 instrument objects", async () => {
    ...
    expect(response.body[0].questionnaires).toEqual(questionnairesReturned[0].questionnaires);
  });
  ...
  const questionnairesReturned = [
    {
      ...
      questionnaires: [
        QuestionnaireHelper.Questionnaire(
          ...
          "July 2020",
          "IPS",
          "https://external-web-url/IPS2007T?LayoutSet=CATI-Interviewer_Large",
        ),
      ],
    },
    {
...
const feature = loadFeature("./src/client/features/TO_Interviewer_Happy_Path.feature", {
  tagFilter: "@server",
});
...
defineFeature(feature, (test) => {
  ...
  const questionnaireHasATelOpsStartDateOfToday = (given: DefineStepFunction) => {
    given("a survey questionnaire has a TelOps start date of today", async () => {
      mock
        .onGet(liveDateUrl)
        .reply(200, { tostartdate: IsoDateHelper.today() }, { "content-type": "application/json" });
    });
  };
  ...
  const questionnaireHasATelOpsStartDateInThePast = (given: DefineStepFunction) => {
    given("a survey questionnaire has a TelOps start date in the past", async () => {
      mock
        ...
        .reply(
          200,
          { tostartdate: IsoDateHelper.yesterday() },
  ...
  const questionnaireHasATelOpsStartDateInTheFuture = (given: DefineStepFunction) => {
    given("a survey questionnaire has a TelOps start date is in the future", async () => {
      mock
        ...
        .reply(
          200,
          { tostartdate: IsoDateHelper.tomorrow() },
          { "content-type": "application/json" },
        );
    });
  };
  ...
  const thenIWillSeeTheQuestionnaireListed = (then: DefineStepFunction) => {
    then("I will see that questionnaire listed for the survey", () => {
      ...
      const questionnaireListReturned = [
        QuestionnaireHelper.Questionnaire(
          questionnaireName,
          true,
          "July 2020",
  ...
  test("Show surveys that have a TelOps start date of today and an active survey day in TOBI", ({
    given,
    and,
    when,
    then,
  ...
  test("Show surveys that have a TelOps start date in the past and an active survey day in TOBI", ({
    given,
    and,
    when,
    then,
  ...
  test("Do not show surveys that have an active survey day but TelOps start date in the future in TOBI", ({
    given,
    and,
    when,
    then,
  ...
  test("Do not show surveys that have a TelOps start date in the past but no active survey day in TOBI", ({
    given,
    and,
    when,
    then,
  ...
  test("Show surveys that do not have a TelOps start date but have an active survey day in TOBI", ({
    given,
    and,
    when,
    then,
  ...
  test("Do not show surveys that do not have a TelOps start date and do not have an active survey day in TOBI", ({
    given,
    and,
    when,
    then,
